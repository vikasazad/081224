...
export const generateTimeline = (
  ...
  for (let i = -15; i <= 15; i++) {
    ...
    timeline.push({
      ...
      isFuture,
      bookingId:
        statusPatterns[i] === "booked" || statusPatterns[i] === "occupied"
          ? `BK-${99000 + Math.abs(i)}`
          : undefined,
      guestName:
        statusPatterns[i] === "booked" || statusPatterns[i] === "occupied"
  ...
  for (let i = -15; i <= 15; i++) {
    ...
    if (hasBooking) {
      ...
      checkOutDate.setDate(
        checkOutDate.getDate() + Math.floor(Math.random() * 4) + 1,
      );
      ...
      entries.push({
        ...
        status:
          i < 0
            ? "confirmed"
            : statusOptions[Math.abs(i) % statusOptions.length],
        paymentStatus:
          paymentStatusOptions[Math.abs(i) % paymentStatusOptions.length],
        paymentMode:
          paymentModeOptions[Math.abs(i) % paymentModeOptions.length],
...
export const alertNotifications: AlertNotification[] = [
  {
    ...
    description:
      "Housekeeping flagged a linen update for the master suite. Verification needed before next check-in at 2:00 PM.",
    image: "/images/alerts/inspection.jpg",
    deadline: "2:00 PM",
  },
];

export const getRoomDashboardData = (
  roomId: string,
): RoomDashboardData | null => {
...
export const exportToCSV = (
  ...
  filename: string = "room-activity-log.csv",
): void => {
  const headers = [
    "Reservation ID",
    "Guest Name",
