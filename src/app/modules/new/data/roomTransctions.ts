import {
  RoomTransactionsData,
  TransactionItem,
  FolioSummary,
  GuestInfo,
  SystemInfo,
} from "../types/roomTransctions";
...
export const getRoomTransactionsData = (
  roomId: string,
): RoomTransactionsData => {
...
export const calculateFolioSummary = (
  transactions: TransactionItem[],
): FolioSummary => {
  const runningTotal = transactions.reduce(
    (sum, txn) => sum + txn.finalAmount,
    0,
  );
  const appliedCoupons = transactions.reduce(
    (sum, txn) => sum + (txn.amount - txn.finalAmount),
    0,
  );
  const taxRate = 10;
